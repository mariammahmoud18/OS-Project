...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
    ...
    {
        //pf_add_env_page(e,virtual_address,NULL);
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
    ...
    uint32 New_va=virtual_address;
    int i=0;
    while(i<ROUNDUP(size,PAGE_SIZE)/PAGE_SIZE)
    {
        pf_remove_env_page(e,New_va);
        New_va +=PAGE_SIZE;
        i++;
    }
    ////////////////////////////////////////////////////////
    ...
    uint32*table=NULL;
    struct Frame_Info *entry=NULL;
  //  struct Frame_Info *frame2=NULL;
    /////////////////////
    //FREE IN ACTIVE LIST
    LIST_FOREACH(element,&(e->ActiveList))
    {
        if((element->virtual_address<(virtual_address+size)) && (element->virtual_address>=virtual_address))
        {
             get_page_table(curenv->env_page_directory,(void*)element->virtual_address,&ptr_page_table);
            ...
            for(int i=0; i<PAGE_SIZE/4; i++)
            {
                ...
                if(c==PAGE_SIZE/4)
                {
                	entry=get_frame_info(e->env_page_directory,(void*)ptr_page_table,&table);
                	entry->references=0;
                    free_frame(entry);
                    pd_clear_page_dir_entry(e,element->virtual_address);
                }
            }
            ...
            {
                struct WorkingSetElement*element2=LIST_FIRST(&(e->SecondList));
                LIST_REMOVE(&(e->SecondList),element2);
                pt_set_page_permissions(e, element2->virtual_address, PERM_PRESENT, 0);
                LIST_INSERT_TAIL(&(e->ActiveList),element2);
            }
