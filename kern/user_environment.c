...
//	__remove_pws_user_pages(e);
    ...
    for(int i=0; i<PDX(USER_TOP); i++)
    {
        ...
        {
            ptr_frame_info=to_frame_info(e->env_page_directory[i]);
    ...
    to_frame_info((int)e->env_page_directory-KERNEL_BASE)->references=0;
    free_frame(to_frame_info((int)e->env_page_directory-KERNEL_BASE));
...
struct ProgramSegment* PROGRAM_SEGMENT_NEXT(struct ProgramSegment* seg, uint8* ptr_program_start)
{
    ...
    while (ph[(*seg).segment_id].p_type != ELF_PROG_LOAD && ((*seg).segment_id < pELFHDR->e_phnum))
        (*seg).segment_id++;
